...
with torch.profiler.profile(
    schedule=torch.profiler.schedule(wait=20, warmup=20, active=2, repeat=1),
    on_trace_ready=torch.profiler.tensorboard_trace_handler(
        "/local_scratch/musil/nl_n.prof"
    ),
    record_shapes=False,
    ...
    for _ in range(50):
        mapping, mapping_batch, shifts_idx = compute_neighborlist_n2(
            cutoff, pos, cell, pbc, batch
        )
        prof.step()
