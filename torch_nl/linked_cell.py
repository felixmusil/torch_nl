...
def get_linear_bin_idx(cell: torch.Tensor, pos: torch.Tensor, nbins_s: torch.Tensor) -> torch.Tensor:
...
def linked_cell(
    ...
    cell_shifts = compute_cell_shifts(cell, shifts_idx, batch_image)
    ...
    images -= i_min - 1e-5
    i_pos = pos - i_min - 1e-5
    box_length = images.max(dim=0).values - i_min + 1e-3
