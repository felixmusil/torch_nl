...
frame = make_supercell(frame, 5*np.eye(3))
...
with torch.profiler.profile(
            ...
            on_trace_ready=torch.profiler.tensorboard_trace_handler('nl_n.prof'),
            record_shapes=False,
            profile_memory=False,
            with_stack=True,
        ) as prof:
    for _ in range(50):
        mapping, mapping_batch, shifts_idx = compute_neighborlist(cutoff, pos, cell, pbc, batch)
        prof.step()